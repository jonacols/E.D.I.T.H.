...
MOT_DE_PASSE_ARTHUR = get_secret("PASSWORD_ARTHUR")   # None si absent -> accès impossible
...
MAX_CONTEXT_MESSAGES = 30          # fenêtre glissante (compression au-delà)
SEUIL_PERTINENCE_DEFAUT = 1.25     # distance L2 Chroma/MiniLM ; plus petit = plus pertinent
SEUIL_DOUBLON = 0.12               # en dessous -> souvenir quasi identique, on ignore
...
MODEL_ROUTER     = "google/gemini-3.6-flash"   # routeur économique
...
# Prix de secours ($/token) si l'API catalogue est injoignable — indicatifs
PRIX_DEFAUT = {
...
# ================= 4. THÈME & UI (inchangé, comme demandé) =================
...
# Balises PWA (iPhone : Réglages Safari -> "Sur l'écran d'accueil")
components.html("""
<script>
try {
...
def sauvegarder_souvenir(info, chat_id=None, projet=None, manuel=False):
    """Retourne (date_fr, créé_bool). Déduplique et sauvegarde un backup."""
    if memoire_collection is None:
        return date_fr_courte(), False
    try:
        res = memoire_collection.query(query_texts=[info], n_results=1)
        dists = res.get("distances", [[]])[0]
        if dists and dists[0] < SEUIL_DOUBLON:
            return date_fr_courte(), False
    except Exception:
        pass
...
def recuperer_souvenirs(prompt, projet_actif="Général"):
    ...
    try:
        res = memoire_collection.query(query_texts=[prompt], n_results=6)
    except Exception:
        return ""
    ...
    return ("\n\n# SOUVENIRS DATÉS (mémoire vectorielle) :\nUtilise-les s'ils sont pertinents ; "
            "les dates te permettent de construire des chronologies.\n" + "\n".join(lignes))
...
def charger_historique():
    return lire_json(FICHIER_HISTORIQUE, {})

def sauvegarder_historique():
    ecrire_json_atomique(FICHIER_HISTORIQUE, st.session_state.chats)
...
def create_new_chat(is_first_ever=False):
    ...
    st.session_state.chats[nid] = {
        ...
        "messages": messages,
        "projet": st.session_state.get("widget_projet", "Général"),
        "titre_manuel": False,
    }
...
def get_smart_route(prompt_text, has_image=False, contexte=""):
    try:
        ...
        r = client.chat.completions.create(
            ...
            messages=[{"role": "system", "content":
                       "Classifie la demande par UN seul mot : "
                       "SIMPLE (question factuelle, courte, quotidien), "
                       "COMPLEXE (analyse, raisonnement, maths, longue réponse), "
                       "CODE (programmation, débogage, électronique technique), "
                       "CREATIVE (rédaction, brainstorming), "
                       "SENSITIVE (sujet polémique ou peu filtré)."},
    ...
    if PROFIL == "padre":  # plafonné, jamais de Grok
        ...
        if "CODE" in d:
            choix, raison = MODEL_CODE, "Programmation → Kimi K3"
        elif "COMPLEXE" in d:
            choix, raison = MODEL_HEAVY, "Requête complexe → modèle lourd"
        elif "CREATIVE" in d:
            choix, raison = MODEL_CREATIVE, "Créatif → Grok"
        elif "SENSITIVE" in d:
            choix, raison = MODEL_UNFILTERED, "Sujet sensible → Grok"
        else:
            choix, raison = MODEL_LIGHT, "Requête standard → modèle rapide"
...
def obtenir_resume(chat):
    ...
    try:
        r = client.chat.completions.create(
            model=MODEL_LIGHT,
            messages=[{"role": "system", "content":
                       "Résume cette conversation en 5 à 8 puces factuelles (sujets, décisions, informations clés). "
...
def generer_titre_automatique(chat):
    ...
    try:
        ...
        pass
    if chat["title"] == "Nouvelle discussion":  # secours si l'appel a échoué
...
# ================= 17. STREAMING & FALLBACK =================
def stream_edith(box, candidats, api_messages):
    """Essaie les modèles dans l'ordre. Retourne (texte, usage, modèle_utilisé, a_fallback)."""
...
MOTS_REFUS = ["je ne peux pas vous aider avec", "i cannot assist", "i can't assist",
...
def bulle_user(texte):
    st.markdown(f'<div class="msg-row user"><div class="msg-bubble">{html.escape(texte)}</div></div>',
                unsafe_allow_html=True)

def tete_edith():
    st.markdown('<div class="msg-row edith"><div class="edith-avatar">⚡</div><div class="edith-nom">E.D.I.T.H.</div></div>',
                unsafe_allow_html=True)
...
with st.sidebar:
    ...
    if mode_choisi == "🎛️ Manuel":
        selected_manual_model = MODELS_PROFIL[st.selectbox(
            "Choisir l'IA :", list(MODELS_PROFIL.keys()), label_visibility="collapsed")]

    if not st.session_state.incognito:
        st.markdown('<div class="side-label">Projet</div>', unsafe_allow_html=True)
        projets = liste_projets()
        if st.session_state.get("widget_projet") not in projets:
            st.session_state["widget_projet"] = "Général"
        col_proj, col_add = st.columns([0.78, 0.22])
        with col_proj:
            projet_actif = st.selectbox("Projet actif", projets, key="widget_projet",
                                        label_visibility="collapsed")
        with col_add:
            with st.popover("➕"):
                nom_projet = st.text_input("Nom du projet :", key="input_nouveau_projet",
                                           placeholder="Voiture RC, Drone…")
                if st.button("Créer le projet", use_container_width=True):
                    if nom_projet.strip():
                        ajouter_projet(nom_projet.strip())
                        st.session_state["widget_projet"] = nom_projet.strip()
                        st.rerun()
        ...
        for c_id in reversed(list(st.session_state.chats.keys())):
            ...
            if data.get("projet", "Général") != projet_actif:
                continue
            if not chat_matches_search(data, search_query):
                continue
            ...
            with col_title:
                if st.button(label_titre, key=f"btn_chat_{c_id}", use_container_width=True):
                    ...
                    st.session_state["widget_projet"] = data.get("projet", "Général")
                    st.rerun()
            with col_menu:
                with st.popover("⋮"):
                    ...
                    if st.button("✏️ Modifier le titre", key=f"rename_btn_{c_id}", use_container_width=True):
                        if nouveau_titre.strip():
                            data["title"] = nouveau_titre.strip()
                            data["titre_manuel"] = True
                            sauvegarder_historique()
                            st.rerun()
                    st.download_button("⬇️ Exporter (.md)", data=exporter_markdown(data),
                                       file_name=re.sub(r"[^\w\- ]", "", data["title"])[:30] + ".md",
                                       mime="text/markdown", key=f"export_{c_id}", use_container_width=True)
                    st.divider()
                    confirmation = st.checkbox("Confirmer la suppression", key=f"confirm_del_{c_id}")
                    if st.button("🗑️ Supprimer chat & mémoires", key=f"del_chat_{c_id}",
                                 use_container_width=True, disabled=not confirmation):
    ...
    with st.popover("➕ Nouveau rappel", use_container_width=True):
        ...
        if st.button("Ajouter le rappel", use_container_width=True):
            if r_msg.strip():
                ajouter_rappel(r_date.isoformat(), r_heure, r_msg.strip())
                st.toast("Rappel ajouté ⏰")
                st.rerun()
    rappels_a_venir = sorted([r for r in charger_rappels() if not r.get("fait")],
                             key=lambda x: (x["date"], x.get("heure", "")))[:6]
    for r in rappels_a_venir:
        en_retard = r["date"] <= date_iso()
        c_txt, c_del = st.columns([0.85, 0.15])
        with c_txt:
            st.caption(f"{'🔴' if en_retard else '⚪'} {r['date'][8:10]}/{r['date'][5:7]} "
                       f"{r.get('heure','')} — {r['message'][:38]}")
        with c_del:
            if st.button("🗑️", key=f"del_rappel_{r['id']}"):
                supprimer_rappel(r["id"])
                st.rerun()
    ...
    if memoire_collection is not None:
        ...
        st.markdown(f'<div class="mem-count">🧠 {nb} souvenir{"s" if nb > 1 else ""} dans la mémoire absolue</div>',
                    unsafe_allow_html=True)
        if PROFIL == "arthur":
            with st.expander("🛠️ Gérer la mémoire manuellement"):
                with st.form("form_ajout_memoire", clear_on_submit=True):
                    nouvelle_info = st.text_input("Ajouter un souvenir :",
                                                  placeholder="ex: Arthur aime coder tard la nuit")
                    if st.form_submit_button("➕ Ajouter", use_container_width=True):
                        if nouvelle_info.strip():
                            fr, cree = sauvegarder_souvenir(nouvelle_info.strip(), manuel=True,
                                                            projet=st.session_state.get("widget_projet", "Général"))
                            st.toast(f"Souvenir ajouté le {fr} !" if cree else "Doublon ignoré.", icon="🧠")
                            st.rerun()
                        ...
                        with c_del:
                            if st.session_state.get("confirm_mem") == s_id:
                                if st.button("✅", key=f"cfm_mem_{s_id}"):
                                    memoire_collection.delete(ids=[s_id])
                                    st.session_state.pop("confirm_mem", None)
                                    backup_memoire()
                                    st.toast("Souvenir supprimé !", icon="🗑️")
                                    st.rerun()
                            else:
                                if st.button("🗑️", key=f"del_mem_{s_id}"):
                                    st.session_state.confirm_mem = s_id
                                    st.rerun()
                else:
                    st.caption("Aucun souvenir dans la mémoire.")

    # --- Budget (Arthur uniquement) ---
    if PROFIL == "arthur":
        with st.expander("💶 Budget du mois"):
            dep = lire_json(FICHIER_DEPENSES, {})
            m = dep.get(datetime.now().strftime("%Y-%m"),
        ...
        with st.expander("🛠️ Reroutage & Modèles"):
            ...
            if st.button("📡 Lister les modèles OpenRouter", use_container_width=True):
                if REQUESTS_DISPONIBLE:
                    try:
                        ...
                        st.error(f"Catalogue inaccessible : {e}")
            if st.session_state.get("modeles_or"):
                ...
                for ligne in st.session_state.modeles_or:
                    if not filtre or filtre.lower() in ligne.lower():
                        st.caption(ligne)
...
if not incognito:
    ...
    if dus:
        for r in dus:
            ...
            with c_txt:
                st.warning(f"⏰ **Rappel** : {r['message']} (prévu le {r['date'][8:10]}/{r['date'][5:7]} {r.get('heure','')})")
            with c_ok:
                if st.button("✅ Fait", key=f"fait_{r['id']}"):
                    marquer_rappel_fait(r["id"])
                    st.rerun()
...
if chat.get("projet", "Général") != "Général" and not incognito:
    pills += f'<span class="pill">📁 {html.escape(chat["projet"])}</span>'
if st.session_state.web_actif:
    pills += '<span class="pill">🌐 Web</span>'
if incognito:
    pills += '<span class="pill">🕶️ Incognito</span>'
pills += f'<span class="pill">📅 {date_complete()}</span>'
st.markdown(f'<div class="topbar"><span class="chat-title">{html.escape(chat["title"])}</span>'
            f'<span>{pills}</span></div>', unsafe_allow_html=True)
st.divider()
...
if not messages:
    if incognito:
        ...
        <div class="hero">
          ...
          <h1>Session incognito.</h1>
          <p>Rien ne sera conservé. Même moi, je ferai semblant d'oublier.</p>
...
for idx, m in enumerate(messages):
    if m["role"] == "user":
        ...
        if m.get("document_nom"):
            st.caption(f"📎 {m['document_nom']}")
    else:
        tete_edith()
        st.markdown(m["content"])
        if "metadata" in m:
            ligne_statut(m["metadata"])
...
if messages and messages[-1]["role"] == "assistant" and messages[-1].get("content"):
    with st.popover("🔁 Régénérer cette réponse"):
        ...
        if st.button("🔁 Régénérer", use_container_width=True, key="btn_regen"):
            messages.pop()
            st.session_state.regen_demandee = True
            st.session_state.regen_model = None if choix_regen.startswith("🤖") else MODELS_PROFIL[choix_regen]
            if not incognito:
                sauvegarder_historique()
            st.rerun()

# ================= 20. ENTRÉES & TRAITEMENT =================
suffixe_up = f"{st.session_state.current_chat_id}_{st.session_state.upload_counter}"
col_up1, col_up2, col_up3 = st.columns([1, 1, 1])
with col_up1:
    uploaded_image = st.file_uploader("🖼️ Image", type=["png", "jpg", "jpeg", "webp"],
                                      key=f"uploader_img_{suffixe_up}")
with col_up2:
    audio_val = st.audio_input("🎤 Parler à E.D.I.T.H.")
with col_up3:
    types_doc = ["txt", "md"] + (["pdf"] if PDF_DISPONIBLE else [])
    uploaded_doc = st.file_uploader("📎 Document", type=types_doc, key=f"uploader_doc_{suffixe_up}",
                                    help="PDF, TXT ou MD. Petit fichier : injecté directement. Gros : indexé pour recherche.")
            ...
            with st.spinner("Transcription en cours (Whisper)..."):
                try:
                    transcript = openai_client.audio.transcriptions.create(
                        model="whisper-1", file=("audio.wav", audio_val))
...
if (prompt and prompt.strip()) or regen_actif:
    ...
    if regen_actif:
        ...
        if uploaded_doc:
            ...
            if texte_doc is None:
                st.error("Lecture du document impossible (pypdf est-il installé ?).")
            elif not texte_doc.strip():
                st.warning("Document vide ou texte non extractible (PDF scanné ?).")
            else:
                user_msg["document_nom"] = uploaded_doc.name
                if len(texte_doc) <= 12000:
                    user_msg["document"] = texte_doc
                else:
                    nb_chunks = indexer_document(st.session_state.current_chat_id,
                                                 uploaded_doc.name, texte_doc)
        ...
        if user_msg.get("document_nom"):
            st.caption(f"📎 {user_msg['document_nom']}")

    # --- Titre automatique après le premier échange ---
    if not incognito and not regen_actif and not chat.get("titre_manuel"):
        ...
        if chat["title"] == "Nouvelle discussion" and nb_echanges <= 1:
            pass  # on attend la réponse pour générer le titre juste après
    ...
    with st.spinner("Analyse de la demande…"):
        if regen_actif and st.session_state.get("regen_model"):
            ...
            selected_model, route_reason = get_smart_route(
                prompt_text, user_msg.get("image_b64") is not None, contexte_routeur)

    web = st.session_state.web_actif
    def avec_online(m):
        return m + ":online" if web else m
    ...
    try:
        ...
        if saves:
            if incognito:
                ...
                for info in saves:
                    fr, cree = sauvegarder_souvenir(info.strip(),
        ...
        rappels_detectes = re.findall(
            ...
            texte_propre, re.IGNORECASE)
        if rappels_detectes:
            if incognito:
                st.toast("Mode incognito : rappel non enregistré.", icon="🕶️")
            else:
        ...
        st.session_state.upload_counter += 1  # vide les uploaders -> l'image ne se réattache plus
        st.rerun()
